package com.libgdx.cookbook.box2d;
...
public class BodyEditorLoader {
    ...
    private RigidBodyModel readRigidBody(JsonValue bodyElem) {
        ...
        for (JsonValue vertices = polygons.child(); vertices != null; vertices = vertices
                .next()) {
            PolygonModel polygon = new PolygonModel();
            rbModel.polygons.add(polygon);

            for (JsonValue vertex = vertices.child(); vertex != null; vertex = vertex
                    .next()) {
                polygon.vertices.add(new Vector2(vertex.getFloat("x"), vertex
                        .getFloat("y")));
            }
        ...
        for (JsonValue circle = circles.child(); circle != null; circle = circle
                .next()) {
