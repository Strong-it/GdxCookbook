...
public class WidgetsSample extends BaseScreen {

    private static final String TAG = WidgetsSample.class.getSimpleName();
    ...
    private ButtonGroup<CheckBox> bg;
    ...
    public void show() {
        ...
        image.setPosition(middlepointX-image.getWidth()*0.5f, label.getY()-image.getHeight() - 10); // 相对于label下移10个像素
        
        float firstRowY = image.getY() - 50; // 确认第一个button Y坐标

        // Button with background Button各个属性可以为null
        ...
        // Button with background and text  TextButtonStyle的font必须初始化，其余可以为null, 里面有一个Label
...
//        imageTextButton.debug();  // 设置单个Actor显示绿框,但是只显示ImageTextButton里面的内容 

        //Grouped checkbox buttons  (checkboxOn  checkBoxOff 这个必须得初始化)
        ...
        bg = new ButtonGroup<CheckBox>(); // 把所有的checkbox放在一组管理,并且设置最多有一组能被选中
        ...
        //Single line of text input for passwords  密码输入框
        ...
        tf.setPasswordCharacter('*');   // 将输入文字变为星号
        ...
        listS.fontColorUnselected = Color.RED;
        ...
        dialog.getContentTable().row().colspan(1).center(); // 一行一列居中
        dialog.getContentTable().add(message);         
        dialog.row().colspan(2);  // 一行2列
        dialog.button(tb1);
        dialog.button(tb2);
        dialog.setModal(false);  // 如果设置为true那么为Modal模式，其余的Actor不能被点击
        ...
        window.debug(); // 在window里面的Actor都会画出调试框
        ...
        stage.setDebugAll(true);  // 设置此属性，每个Actor都会有边框
    }
