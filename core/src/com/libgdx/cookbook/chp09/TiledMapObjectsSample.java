...
public class TiledMapObjectsSample extends BaseScreen {
    ...
    private Vector2 direction;  // 控制camera的运动方向
    ...
    public void show() {
        ...
        renderer = new OrthogonalTiledMapRenderer(map, batch); // 如果不创建Batch，那么就会使用默认的
    ...
    private void processMapMetadata() {
        // Load music
        String songPath = map.getProperties().get("music", String.class);  // Tiledmap里面保存音乐的路径
        ...
        MapObjects objects = map.getLayers().get("objects").getObjects(); // 获取Objects layer

        for (MapObject object : objects) {
            String name = object.getName();
            String[] parts = name.split("[.]");  // 区分item.coin   item.key
            RectangleMapObject rectangleObject = (RectangleMapObject) object;  // Represents a rectangle shaped map object  查询代表矩形的postion  size
            Rectangle rectangle = rectangleObject.getRectangle();

            System.out.println("Object found");
            System.out.println("- name: " + name);
            System.out.println("- position: (" + rectangle.x + ", "
                    + rectangle.y + ")");
            System.out.println("- size: (" + rectangle.width + ", "
                    + rectangle.height + ")");
            ...
            if (name.equals("enemy")) {
                ...
                trigger.setPosition(rectangle.x - rectangle.width * 0.5f,
    ...
    private void updateCamera() {
        ...
        camera.position.x = MathUtils.clamp(camera.position.x, cameraMinX,
                cameraMaxX);
        camera.position.y = MathUtils.clamp(camera.position.y, cameraMinY,
                cameraMaxY);
